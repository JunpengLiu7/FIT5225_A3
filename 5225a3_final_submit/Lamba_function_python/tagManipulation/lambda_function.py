...
def extract_key_from_url(url):
    ...
    key_output = parsed_url.path.strip('/')
    print('this is the key_output', key_output)
    return key_output

def check_user_ownership(key, username):
    print(key.startswith(f"images/{username}/"))
...
def lambda_handler(event, context):
    ...
    # Send SNS notifications based on the tag manipulation
    message = f"{username} manipulated images with tags {', '.join(tags)}"
    for tag in tags:
        try:
            sns_client.publish(
                TopicArn=TOPIC,
            ...
            error_message = f'Error publishing message for tag {tag}: {str(e)}'
    ...
    return {
        'statusCode': 201,
        'body': json.dumps('Tags updated successfully and notifications sent')
    }
